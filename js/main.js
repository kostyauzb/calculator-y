...
function init() {
  ...
  const elThemeSwitcherControlWrapper = document.querySelector(
    ".theme-switcher__control-wrapper"
  );
  const elSwitchAudio = document.querySelector("#switch-audio");

  function switchTheme() {
    const elCheckedThemeInput =
      elThemeSwitcherControlWrapper.querySelector("input:checked") ||
      elThemeSwitcherControlWrapper.querySelector("[checked]");
  ...
  function resetThemeInputs() {
    const elsThemeInput = elThemeSwitcherControlWrapper.querySelectorAll(
  ...
  function updateThemeInput(theme) {
    ...
    document
      .querySelector(`.theme-switcher__radio[value='${elCurrentTheme}']`)
  ...
  function handleNumberKeyClick(elKey) {
    ...
    const isKeyDecimalAndCurrentValueEmpty =
      isKeyDecimal && isCurrentValueEmpty;
    const isKeyDecimalAndHasCurrentValueDecimal =
      isKeyDecimal && hasCurrentValueDecimal;
    const hasThreeDecimals =
      hasCurrentValueDecimal &&
      CALCULATOR.currentValue.split(".")[1].length === 3;
  ...
  if (elCalculatorKeyboard) {
    elCalculatorKeyboard.addEventListener(
      "click",
      handleCalculatorKeyboardClick
    );
  }

  /////////////////////////////////////////////////// global event listersn
  document.addEventListener("keyup", function (evt) {
    // theme changing via hotkeys
    const isThemeHotkey =
      evt.altKey && Object.keys(CALCULATOR.themes).includes(evt.key);
    const isEscKey = evt.code === "Escape";
    const isNumberKey =
      CALCULATOR.numberKeyCodes.includes(evt.code) && !evt.shiftKey;
    const isOperatorKey =
    ...
    if (isNumberKey) {
      ...
      emulateKeyPress(
        elCalculatorKeyboard.querySelector(`[data-key="${key}"]`)
      );
    }
